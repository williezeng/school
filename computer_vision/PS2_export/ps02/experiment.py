...
def draw_tl_center(image_in, center, state):
    ...
    cv2.circle(image_copy, center, 1, (0, 0, 0), 2)
    adjusted = (center[0] + 2, center[1])
    cv2.putText(image_copy, text=str(center) + ', ' + state, org=adjusted, fontFace=cv2.FONT_HERSHEY_PLAIN,
...
def mark_traffic_signs(image_in, signs_dict):
    ...
    if signs_dict:
        for k,center in signs_dict.items():
            if k == 'traffic_light':
                cv2.circle(image_copy, center, 1, (0, 0, 0), 2)
                adjusted = (center[0] - 100, center[1] + 40)
                cv2.putText(image_copy, text=str(center) + ', traffic_light', org=adjusted, fontFace=cv2.FONT_HERSHEY_PLAIN,fontScale=1.1, color=(0, 0, 0))
            else:
                cv2.circle(image_copy, center, 1, (0, 0, 0), 2)
                adjusted = (center[0] - 35, center[1] + 20)
                cv2.putText(image_copy, text=str(center), org=adjusted, fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=1.1, color=(0, 0, 0))
    return image_copy
