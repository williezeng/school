...
def part_5a():
    ...
    u10, v10 = ps4.hierarchical_lk(shift_0, shift_r10, levels, k_size, k_type,
                                   sigma, interpolation, border_mode)
    warp = ps4.warp(shift_r10, u10, v10, interpolation, border_mode)

    warper_1 = (0.2)*shift_0 + (0.8)*warp
    warper_2 = (0.4)*shift_0 + (0.6)*warp
    warper3 = (0.6)*shift_0 + (0.4)*warp
    warper4 = (0.8)*shift_0 + (0.2)*warp
    # 2 rows and 3 columns
    height, width = shift_r10.shape
    canvas = np.ones((2*height, 3*width))
    canvas[0 : height] = np.hstack((shift_0, warper4, warper3))
    canvas[height : 2*height] = np.hstack((warper_2, warper_1, shift_r10))

    cv2.imwrite(os.path.join(output_dir, "ps4-5-1-a-1.png"), ps4.normalize_and_scale(canvas))
...
def part_5b():
    ...
    interpolation = cv2.INTER_CUBIC  # You may try different values
    border_mode = cv2.BORDER_REFLECT101  # You may try different values
    u10, v10 = ps4.hierarchical_lk(mc01, mc02, levels, k_size, k_type,
                                   sigma, interpolation, border_mode)
    warp = ps4.warp(mc02, u10, v10, interpolation, border_mode)

    warper1 = (0.2)*mc01 + (0.8)*warp
    warper2 = (0.4)*mc01 + (0.6)*warp
    warper3 = (0.6)*mc01 + (0.4)*warp
    warper4 = (0.8)*mc01 + (0.2)*warp
    # 2 rows and 3 columns
    height, width = mc02.shape
    canvas = np.ones((2*height, 3*width))
    canvas[0 : height] = np.hstack((mc01, warper4, warper3))
    canvas[height : 2*height] = np.hstack((warper2, warper1, mc02))

    cv2.imwrite(os.path.join(output_dir, "ps4-5-1-b-1.png"), ps4.normalize_and_scale(canvas))
    ...
    interpolation = cv2.INTER_CUBIC  # You may try different values
    border_mode = cv2.BORDER_REFLECT101  # You may try different values
    u10, v10 = ps4.hierarchical_lk(mc02, mc03, levels, k_size, k_type,
                                   sigma, interpolation, border_mode)
    warp = ps4.warp(mc03, u10, v10, interpolation, border_mode)

    warper1 = (0.2)*mc02 + (0.8)*warp
    warper2 = (0.4)*mc02 + (0.6)*warp
    warper3 = (0.6)*mc02 + (0.4)*warp
    warper4 = (0.8)*mc02 + (0.2)*warp
    # 2 rows and 3 columns
    height, width = mc03.shape
    canvas = np.ones((2*height, 3*width))
    canvas[0 : height] = np.hstack((mc02, warper4, warper3))
    canvas[height : 2*height] = np.hstack((warper2, warper1, mc03))

    cv2.imwrite(os.path.join(output_dir, "ps4-5-1-b-2.png"), ps4.normalize_and_scale(canvas))
