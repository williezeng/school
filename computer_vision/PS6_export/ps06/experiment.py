...
def visualize_mean_face(x_mean, size, new_dims):
    ...
    reshape_img = np.reshape(x_mean,size)
...
def part_1c():
    p = 0.66  # Select a split percentage value
    k = 33  # Select a value for k
...
def part_2a():
    ...
    rand_accuracy = (np.sum(rand_y==ytrain)/len(ytrain)) * 100
    ...
    wk_accuracy = (np.sum(wk_results==ytrain)/len(ytrain)) * 100
...
def part_3a():
    ...
    # 1 rows, 2 column
    feature1 = ps6.HaarFeature((1, 2), (10, 25), (50, 100))
...
def part_4_a_b():
    ...
    vj_accuracy = ((np.sum(predictions == labels))/len(labels)) * 100
    ...
    vj_accuracy = None
    vj_accuracy = ((np.sum(predictions == real_labels))/len(real_labels)) * 100
