...
def estimate_next_pos(height, radar, mapFunc, OTHER=None):
    ...
    #
    # OTHER = (PARTICLES, TIMESTAMP, SIGMA, RUDDER)
    SIGMA_SETTER = 100
    ...
    total_y = 0
    for i in range(resample_N):
        if OTHER and len(OTHER) == 4 and OTHER[3]:
            rudder = OTHER[3]
        else:
            rudder = 0
        particles[i].glide(rudder=rudder)
...
def next_angle(height, radar, mapFunc, OTHER=None):
    ...
    updated_particle_holder = []
    new_other_holder = None
    NEW_OTHER = {}
    if not OTHER:
        TIME_STEP = 0
        current_xy_estimate, estimated_OTHER, optionalPointsToPlot = estimate_next_pos(height, radar, mapFunc)
        NEW_OTHER['previous_xy_estimate'] = current_xy_estimate
        NEW_OTHER['estimated_other_tuple'] = estimated_OTHER
        steering_angle = 0
    else: # check time step less than 50
        TIME_STEP = OTHER['TIME']
        if TIME_STEP <= 40:
            current_xy_estimate, estimated_OTHER, optionalPointsToPlot = estimate_next_pos(height, radar, mapFunc, OTHER['estimated_other_tuple'])
            NEW_OTHER['previous_previous_xy_estimate'] = OTHER['previous_xy_estimate']
            NEW_OTHER['previous_xy_estimate'] = current_xy_estimate
            NEW_OTHER['estimated_other_tuple'] = estimated_OTHER
            steering_angle = 0
        elif 40 < TIME_STEP < 200:
            previous_xy_estimate = OTHER['previous_xy_estimate']
            previous_previous_xy_estimate = OTHER['previous_previous_xy_estimate']
            # atan2(y,x)
            Y = previous_xy_estimate[1] - previous_previous_xy_estimate[1]
            X = previous_xy_estimate[0] - previous_previous_xy_estimate[0]
            current_bearing = atan2(Y, X)
            goal_bearing = atan2(-Y, -X)
            steering_angle = goal_bearing - current_bearing
            other_holder = OTHER['estimated_other_tuple'] + (steering_angle,)

            current_xy_estimate, estimated_OTHER, optionalPointsToPlot = estimate_next_pos(height, radar, mapFunc, other_holder)
            NEW_OTHER['previous_previous_xy_estimate'] = OTHER['previous_xy_estimate']
            NEW_OTHER['previous_xy_estimate'] = current_xy_estimate
            NEW_OTHER['estimated_other_tuple'] = estimated_OTHER
    ...
    TIME_STEP += 1
    NEW_OTHER['TIME'] = TIME_STEP
    return steering_angle, NEW_OTHER, optionalPointsToPlot
