...
class Robot:
    def __init__(self, x=0.0, y=0.0, bearing=0.0, max_distance=1.0, max_steering=PI/2, dist_noise=0.05, bearing_noise=0.02):
        self.x = x
        self.y = y
    ...
    def measure_distance_and_bearing_to(self, point, noise=False):
        dist_noise_percent = self.dist_noise
        bearing_noise_percent = self.bearing_noise
        ...
        if noise:
            distance_sigma = dist_noise_percent*distance_to_point
            bearing_sigma = bearing_noise_percent*distance_to_point
