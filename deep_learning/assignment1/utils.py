...
def load_mnist_trainval():
    ...
    train_label, val_label = np.split(label, [int(len(label) * 0.8)])
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if shuffle:
        ...
        for i in index_shuf:
            list1_shuf.append(np.asarray(data[i]))
            list2_shuf.append(np.asarray(label[i]))
        data_new = list1_shuf
        label_new = list2_shuf

    else:
        data_new = data
        label_new = label

    batched_data = [np.asarray(data_new[i:i + batch_size]) for i in range(0, len(data), batch_size)]
    batched_label = [np.asarray(label_new[i:i + batch_size]) for i in range(0, len(data), batch_size)]
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    '''
    plt.figure()
    # plt.xlim(1, len(train_loss_history)+1)
    plt.ylim(0, 1)
    ...
    print(my_path)
    plt.savefig(os.path.join(my_path, 'loss.png'))

    plt.figure()
    plt.xlim(1, len(train_acc_history)+1)
    plt.ylim(0, 1)

    plt.plot(range(1,len(train_acc_history)+1), train_acc_history, label='train acc')
    plt.plot( range(1,len(valid_acc_history)+1), valid_acc_history, label='validation acc')
    plt.ylabel('acc', fontsize=14)
    plt.xlabel('epoch', fontsize=14)
    plt.title('accuracy curve')
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(my_path, 'acc.png'))
