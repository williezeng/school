...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        self.gradients['W1'] = np.dot(X.T, np.multiply(np.dot(dldz2, self.weights['W2'].T), da1dz1))

        self.gradients['W2'] = np.dot(A1.T, dldz2)

        dldz = np.multiply(np.dot(dldz2, self.weights['W2'].T), da1dz1)
        self.gradients['b1'] = np.sum(dldz, axis=0)
        self.gradients['b2'] = np.sum(dldz2, axis=0)
