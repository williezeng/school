...
def train(epoch, data_loader, model, optimizer, criterion):
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        if torch.cuda.is_available():
            data = data.cuda()
            target = target.cuda()
        optimizer.zero_grad()  # Clear out the gradients from the previous step so we don't just keep accumulating them
        out = model(data)  # Get the predictions
        loss = criterion.forward(out, target)  # Check the loss according to expectations
        loss.backward()  # Compute backwards pass (Get gradients)
        optimizer.step()  # Apply optimization logic
